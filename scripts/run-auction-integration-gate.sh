...

{
  ...
  echo ""
  node --test --test-concurrency=1 auctions/auction_core.test.js auctions/auction_flow_remediation.test.js auctions/host_phase4.test.js auctions/lifecycle_worker.test.js
} 2>&1 | tee "$OUT"

if grep -q "ℹ fail 0" "$OUT" && grep -qE "ℹ pass (4[0-9]|5[0-9]|60)" "$OUT"; then
  echo ""
